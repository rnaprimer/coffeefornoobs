...
export default function CategoryForm({ initialData }: CategoryFormProps) {
  ...
  const { register, control, handleSubmit, formState: { errors } } = useForm<CategoryFormData>({
        // @ts-ignore
  ...
  async function onSubmit(data: CategoryFormData) {
    ...
    const result = isEditing 
      ? await updateRecord('categories', initialData.id, submissionData, ['/admin/categories', '/'])
      : await createRecord('categories', submissionData, ['/admin/categories', '/'])
  ...
  return (
    <AdminForm 
      ...
      isSaving={isSaving}
      saveLabel={isEditing ? 'Update Category' : 'Create Category'}
    >
      ...
      <div className="sm:col-span-3">
        <label htmlFor="icon_name" className="block text-sm font-medium leading-6 text-gray-900">Icon Name (Lucide React)</label>
        <div className="mt-2">
          <input
            {...register('icon_name')}
            type="text"
            className="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-amber-600 sm:text-sm sm:leading-6 px-3"
            placeholder="e.g. Coffee, Database"
          />
        </div>
      </div>
      ...
      <div className="sm:col-span-6">
        <label className="block text-sm font-medium leading-6 text-gray-900 mb-2">Icon Image (Overrides Lucide Icon)</label>
        <Controller
          name="icon_media_id"
          control={control}
          render={({ field }) => (
