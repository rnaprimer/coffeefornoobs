...
export default function GuideForm({ initialData }: GuideFormProps) {
  ...
  const { register, control, handleSubmit, formState: { errors } } = useForm<GuideFormData>({
        // @ts-ignore
    ...
    defaultValues: {
      ...
      cover_media_id: initialData?.cover_media_id || '',
  ...
  async function onSubmit(data: GuideFormData) {
    ...
    const result = isEditing 
      ? await updateRecord('guides', initialData.id, submissionData, ['/admin/guides', '/'])
      : await createRecord('guides', submissionData, ['/admin/guides', '/'])
  ...
  return (
    <AdminForm 
      ...
      isSaving={isSaving}
      saveLabel={isEditing ? 'Update Guide' : 'Create Guide'}
    >
      ...
      <div className="sm:col-span-6">
        <label htmlFor="excerpt" className="block text-sm font-medium leading-6 text-gray-900">Excerpt</label>
        <div className="mt-2">
          <textarea
            {...register('excerpt')}
            rows={2}
          ...
          render={({ field }) => (
            <div className="border border-gray-300 rounded-md overflow-hidden">
              <RichEditor 
                ...
                draftKey={`guide-${initialData?.id || 'new'}`}
              />
            </div>
          )}
        />
        {errors.content_json && <p className="mt-2 text-sm text-red-600">{errors.content_json.message as string}</p>}
      </div>
      ...
      <div className="sm:col-span-6">
        <label className="block text-sm font-medium leading-6 text-gray-900 mb-2">Cover Image</label>
        <Controller
          name="cover_media_id"
          control={control}
          render={({ field }) => (
        ...
        />
        {errors.cover_media_id && <p className="mt-2 text-sm text-red-600">{errors.cover_media_id.message}</p>}
      </div>
