...
export default function ProductCard({ name, price, rating, reviews, badge, imageText }: ProductCardProps) {
  return (
    <div className="bg-brand-white border-2 border-brand-dark rounded-xl p-3 flex flex-col hover:shadow-[4px_4px_0px_#111111] transition-all cursor-pointer group h-full">
      ...
      <div className="flex flex-col flex-1 justify-between">
        <div>
          <h3 className="font-bold text-sm leading-snug mb-1">{name}</h3>
        ...
        <div className="flex items-end justify-between mt-2">
          ...
          <button className="w-8 h-8 rounded-full border-2 border-brand-dark flex items-center justify-center hover:bg-brand-lime transition-colors">
            <Plus size={16} />
          </button>
        </div>
      </div>
